...
4. Trag sie ein:
   ...
   ```
   python3 "{{TEAM_SYNC_ROOT}}/scripts/team_sync.py" decide \
   ...
   `--alternative` und `--betrifft` können mehrfach vorkommen,
   `--kontext`, `--begruendung`, `--alternative` und `--betrifft` sind
   optional.
5. Bestätige knapp und nenne den Dateinamen. Die Entscheidung erscheint
   bei allen anderen im Team, gleich mit welchem Programm sie arbeiten.
